...
module.exports = {
    ...
    async execute(interaction) {
        ...
        async function giphyMeme() {
            await fetch('https://api.giphy.com/v1/gifs/random?api_key=Co5kXkrDMOLxcHMWAMTZm4PYWGTgr7Vs&tag=&rating=g').then(async res => {
                let meme = await res.json();
                ...
                return interaction.reply({ embeds: [embed.setTitle(`${title}`).setImage(`${url}`).setURL(url).setColor("Random").setFooter({ text: author, iconURL: pf})] });
            });
        }
        if (platform === "reddit")
            redditMeme();
